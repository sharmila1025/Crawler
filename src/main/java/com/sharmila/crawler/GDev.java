...
public class GDev {
	...
	public void jobCrawl() {
		List<String> empIDList = null;

		try {
			empIDList = FileUtils.readLines(new File("test/employerId.txt"));
			Set<String> empIDSet = new HashSet<>(empIDList);
			for (String empData : empIDSet) {
				String[] emp = empData.split("##");

				Response response = Jsoup.connect("https://www.glassdoor.com/Job/jobs.htm")

						.userAgent("Mediapartners-Google").timeout(10 * 1000).method(Method.POST)
						.data("suggestCount", "0").data("suggestChosen", "false").data("clickSource", "searchBtn")
						.data("typedKeyword", "").data("sc.keyword", emp[1]).data("LocationSearch", emp[2])
						.data("locT", "").data("locId", "").data("jobType", "").referrer("http://www.google.com")
				...
				for (Element e1 : eleMain) {
					...
					for (int i = 1; i < totalPages; i++) {
						...
						if (wholeElement.hasClass("noResultsMessage")
							...
							for (Element e : element) {
								...
								if (empID.equalsIgnoreCase(emp[0])) {
									System.out.println(
											"Employer id from page " + empID + " employer id from file data " + emp[0]);
					...
					System.out.println(
							" The title for the company " + emp[1] + "and location " + emp[2] + " is " + titleSet);
					String titleData = " The title for the company " + emp[1] + "and location " + emp[2] + " is "
							+ titleSet;
